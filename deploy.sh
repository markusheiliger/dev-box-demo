...
purgeResources() {
	...
	for RESOURCEGROUP in $(az group list --subscription $SUBSCRIPTIONID --query '[].name' -o tsv | dos2unix); do
		for WORKSPACE in $(az monitor log-analytics workspace list-deleted-workspaces --resource-group $RESOURCEGROUP --query [].name -o tsv | dos2unix); do
			echo "$SUBSCRIPTIONID - Purging log analytics workspace '$WORKSPACE' ..."
			az monitor log-analytics workspace delete --resource-group $RESOURCEGROUP --name $WORKSPACE --force --yes -o none &
		done
	done 
